...
class App:
    ...
    def create_widgets(self):
        ...
        self.button_save_state_graph = ttk.Button(
            self.job_button_frame, text="Сохранить граф состояний", state=DISABLED,
            command=self.save_state_graph_to_json
        )
        self.button_save_state_graph.grid(row=0, column=4, padx=5, pady=5)
        ...
        simple_radio = ttk.Radiobutton(gradation_frame, text="Простая (0.25, 0.5, 0.75, 1.0)",
                                       variable=self.gradation_type, value="simple")
        simple_radio.grid(row=0, column=0, padx=5, pady=5)

        detailed_radio = ttk.Radiobutton(gradation_frame, text="Детальная (0.1, 0.2, ..., 1.0)",
    ...
    def confirm_action(self):
        return messagebox.askyesno("Подтверждение", "Вы уверены, что хотите обновить характеристики?")
    ...
    def display_categories(self, categories_data):
        ...
        for category, data in sorted_categories:
            ...
            # СОРТИРУЕМ ХАРАКТЕРИСТИКИ ПО НОМЕРУ
            sorted_details = sorted(data['details'], key=lambda x: int(x['char'][1:]) if x['char'][1:].isdigit() else 0)
            sorted_chars = [d['char'] for d in sorted_details]

            categories_text += f"  Характеристики: {', '.join(sorted_chars)}\n"
    ...
    def finish_generation(self):
        ...
        try:
            ...
            if not regex or regex.strip() == "":
                ...
                self.button_save_state_graph.config(state="normal")
    ...
    def save_state_graph_to_json(self):
        """Сохраняет граф состояний в JSON файл с автоматическим именем (компактный формат, с сортировкой по номеру)"""
        if not self.transitions:
            messagebox.showwarning("Предупреждение", "Нет данных для сохранения графа состояний.")
            return

        try:
            # Получаем имя объекта из текстового поля
            object_name = self.name_obj_txt.get("1.0", "end-1c").strip()

            # Если имя объекта пустое, используем "unnamed_object"
            if not object_name:
                object_name = "unnamed_object"

            # Очищаем имя от недопустимых символов для имени файла
            import re
            clean_name = re.sub(r'[<>:"/\\|?*]', '_', object_name)

            # Формируем имя файла с датой и временем
            current_datetime = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            filename = f"state_graph_{clean_name}_{current_datetime}.json"

            # Создаем директорию для графов, если её нет
            graph_dir = "state_graphs"
            if not os.path.exists(graph_dir):
                os.makedirs(graph_dir)

            file_path = os.path.join(graph_dir, filename)

            # Функция для извлечения номера из состояния (S0, S1, S10 и т.д.)
            def get_state_number(state):
                if state == 'Z':
                    return float('inf')  # Z всегда в конце
                try:
                    return int(state[1:])  # S0 -> 0, S10 -> 10
                except:
                    return 0

            # Сортируем состояния по номеру
            states = sorted(self.transitions.keys(), key=get_state_number)

            # Ручное форматирование JSON
            lines = ["{"]

            for i, state in enumerate(states):
                edges = self.transitions[state]

                # Сортируем символы (q1, q2, q3...)
                def get_char_number(char):
                    try:
                        return int(char[1:])  # q1 -> 1, q10 -> 10
                    except:
                        return 0

                symbols = sorted(edges.keys(), key=get_char_number)

                lines.append(f'    "{state}": {{')

                for j, symbol in enumerate(symbols):
                    next_states = edges[symbol]

                    # Приводим к списку
                    if isinstance(next_states, str):
                        next_states = [next_states]

                    # Сортируем целевые состояния по номеру
                    next_states_sorted = sorted(next_states, key=get_state_number)

                    # Форматируем список в одну строку
                    next_states_str = "[" + ", ".join(f'"{s}"' for s in next_states_sorted) + "]"

                    # Добавляем запятую если не последний
                    comma = "," if j < len(symbols) - 1 else ""
                    lines.append(f'        "{symbol}": {next_states_str}{comma}')

                # Закрываем объект состояния
            ...
            messagebox.showinfo("Успех", f"Граф состояний сохранён в файл:\n{file_path}")

        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось сохранить граф состояний: {e}")
    ...
    def reset_all(self):
        ...
        self.button_save_state_graph.config(state=DISABLED)
