...
characteristics = {}
tps = {}
# characteristics = data.characteristics1
...
class App:
    ...
    def start_generation(self):
        ...
        if not tps or len(tps) == 0:
            messagebox.showwarning("Предупреждение", "Нет текстовых потоков для обработки. Пожалуйста, добавьте хотя бы один текстовый поток.")
            return
        
        # Проверка на наличие характеристик
        if not characteristics or len(characteristics) == 0:
            messagebox.showwarning("Предупреждение", "Нет характеристик объекта. Пожалуйста, добавьте характеристики перед генерацией.")
            return
        ...
        if not scripts or len(scripts) == 0:
            messagebox.showwarning("Предупреждение", "Не удалось извлечь скрипты из текстовых потоков. Убедитесь, что слова в текстах совпадают со словами в характеристиках.")
        ...
        if not non_empty_scripts:
            messagebox.showwarning("Предупреждение", "Все скрипты пустые. Убедитесь, что слова в текстах совпадают со словами в характеристиках.")
            self.insert_regex("")  # Очищаем поле
            return
        ...
        try:
            ...
            if not regex or regex.strip() == "":
                messagebox.showwarning("Предупреждение", "Не удалось сгенерировать регулярное выражение. Проверьте данные и попробуйте снова.")
                self.insert_regex("")  # Очищаем поле
            else:
    ...
    def save_object(self):
        ...
        for i, (key, value) in enumerate(tps.items(), start=1):
            ...
            if isinstance(value, tk.Text):
                ...
                # Если это уже строка
                text_content = str(value).strip()
            renamed_dict[renamed_key] = text_content  # Добавляем новый ключ и значение в новый словарь
